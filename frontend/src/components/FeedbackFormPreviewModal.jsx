...
const FeedbackFormPreviewModal = ({ open, onClose, formData, eventTitle }) => {
  const theme = useTheme();

  // Debug logging
  console.log('FeedbackFormPreviewModal - formData:', formData);
  console.log('FeedbackFormPreviewModal - formData type:', typeof formData);
  console.log('FeedbackFormPreviewModal - formData keys:', formData ? Object.keys(formData) : 'null');
  ...
  try {
    if (formData?.schema) {
      ...
      console.log('Created survey model from schema:', formData.schema);
    } else if (formData?.elements || formData?.pages) {

      surveyModel = new Model(formData);
      console.log('Created survey model from formData directly');
    } else {
  ...
  if (surveyModel) {
    try {
      ...
      console.log('Survey model configured successfully:', {
        mode: surveyModel.mode,
        pages: surveyModel.pages?.length,
        questions: surveyModel.getAllQuestions()?.length,
        elements: surveyModel.getAllQuestions()
      });
    } catch (error) {
